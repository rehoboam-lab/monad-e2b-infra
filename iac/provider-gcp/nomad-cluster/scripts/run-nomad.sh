...
function run {
  ...
  if [[ "$server" == "true" ]]; then
    ...
    assert_not_empty "--consul-token-file" "$consul_token"
  ...
  generate_nomad_config "$server" "$client" "$num_servers" "$config_dir" "$user" "$consul_token" "$node_pool" "$orchestrator_job_version" "$node_labels" "$nomad_server_tag_name"

...
"${assertion_script}" \
  "${token}" monad-code-terraform-state monad-code us-east4 worker "${repo}" >/dev/null
...
expect_failure \
  wrong-stage \
  'not bound to this canonical bucket, project, region, stage, and exact source head' \
  "${assertion_script}" "${token}" monad-code-terraform-state \
  monad-code us-east4 api "${repo}"

expect_failure \
  invalid-stage \
  'Invalid network-hardening recovery stage' \
  "${assertion_script}" "${token}" monad-code-terraform-state \
  monad-code us-east4 disabled "${repo}"
...
expect_failure \
  ...
  monad-code-terraform-state monad-code us-east4 worker "${repo}"

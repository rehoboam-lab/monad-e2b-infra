...
grep -F 'var.api_controller_service_account_email != var.worker_build_service_account_email' \
  "${api_pool}" >/dev/null
...
test "$(grep -Fc 'google_service_account_email = var.worker_build_service_account_email' "${cluster_main}")" -eq 2
...
grep -F 'api_controller_service_account_email = module.init.api_controller_service_account_email' "${root_main}" >/dev/null
